...
print('||vec_a||_2 = ', slinalg.norm(vec_a))

# 1ノルム
print('||vec_a||_1 = ', np.linalg.norm(vec_a, 1))
print('||vec_a||_1 = ', slinalg.norm(vec_a, 1))

# 無限大ノルム
print('||vec_a||_inf = ', np.linalg.norm(vec_a, np.inf))
print('||vec_a||_inf = ', slinalg.norm(vec_a, np.inf))
...
print('|| A^(-1) * A - I ||_2 = ', '{:25.17e}'.format(slinalg.norm(ret)))
print('|| A^(-1) * A - I ||_F = ', '{:25.17e}'.format(np.linalg.norm(ret, 'fro')))
print('|| A^(-1) * A - I ||_F = ', '{:25.17e}'.format(slinalg.norm(ret, 'fro')))
print('|| A^(-1) * A - I ||_1 = ', '{:25.17e}'.format(np.linalg.norm(ret, 1)))
print('|| A^(-1) * A - I ||_1 = ', '{:25.17e}'.format(slinalg.norm(ret, 1)))
print('|| A^(-1) * A - I ||_1 = ', '{:25.17e}'.format(np.linalg.norm(ret, np.inf)))
print('|| A^(-1) * A - I ||_1 = ', '{:25.17e}'.format(slinalg.norm(ret, np.inf)))
...
# cond(A) : numpyのみ
h_mat = slinalg.hilbert(3)
inv_h_mat = slinalg.invhilbert(3)
print('Hilbert matrix = \n', h_mat)
print('Inverse Hilbert matrix = \n', inv_h_mat)
print('||H * H^(-1) - I3 = \n', slinalg.norm(h_mat @ inv_h_mat - I3))
...
# det(A)
p_mat = slinalg.pascal(3)
print('Pascal matrix = \n', p_mat)
print('det(P) = |P| = ', slinalg.det(p_mat))
